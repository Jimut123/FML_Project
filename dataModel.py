...
## This can be done in one single function
    ...
    def _init_train_data(self):
        """Record items interated with each user in a dataframe self.clicks, and create adjacency
        matrix self.rating_matrix.
        """
        ## creating a pandas series of clicks
        self.clicks = (
            self.train.groupby(self.col_user)[self.col_item]
            .apply(set)
            .reset_index()
            .rename(columns={self.col_item: self.col_item + "clicked"})
        )
        ## Create the rating matrix using scipys sparse dictionary of keys matrix
        self.rating_matrix = sp.dok_matrix((self.n_users, self.n_items), dtype=np.float32)
        self.rating_matrix[self.train[self.col_user], self.train[self.col_item]] = 1.0
    ...
    def train_loader(self, batch_size):
        ...
        interact = self.clicks.iloc[users]
        pos_items = interact[self.col_item + "clicked"].apply(
            lambda x: random.choice(list(x))
        )
        neg_items = interact[self.col_item + "clicked"].apply(
